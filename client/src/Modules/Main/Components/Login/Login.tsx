...
export default function Login() {
    ...
    const loginUser = async (e: any) => {
        ...
        if (res.payload.response && res.payload.response.status !== 200) {
            if (res.payload.response != undefined  && res.payload.response.status !== undefined  && res.payload.response.status == 606){
                toast.warn(res.payload.response.data.error);
                navigate('/emailvalidation', {state: userName});
                return;
            }
            if (res.payload.response != undefined  && res.payload.response.data !== undefined && res.payload.response.data.error != undefined){
