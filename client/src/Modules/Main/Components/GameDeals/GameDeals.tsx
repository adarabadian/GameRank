...
export default function GameDeals(props: any) {
	...
	const currencies: Array<Currency> = useSelector(
		(state: RootState) => state.currenciesState.currencies
	);
	...
	useEffect(() => {
		...
		if (dealsTable.current != null) dealsTable.current.scrollTo(0, 0);
	}, [platformFilter]);
	...
	async function getGameDeals() {
		if (
			game.deals != [] &&
			game.deals != undefined &&
			game.deals.filter(
				(deal: Deal) => deal.platform == platformFilter
			) != undefined &&
			game.deals.filter(
				(deal: Deal) => deal.platform == platformFilter
			) != [] &&
		...
		try {
			const response = await axios.get(
				"https://gamerank.adarabadian.com/deals",
				{ params: { game: game.name, platform: platformFilter } }
			);
			setDeals(
				response.data.deals.sort(
					(a: Deal, b: Deal) => a.price - b.price
				)
			);

			if (game.deals != undefined) {
				game.deals.push(
					response.data.sort((a: Deal, b: Deal) => a.price - b.price)
				);
			} else {
				game.deals = response.data.sort(
					(a: Deal, b: Deal) => a.price - b.price
				);
			}
	...
	async function getCurrFromServer(requestCurrency: string, index: number) {
		...
		let res: any = await axios.get(
			`https://${host}/latest?amount=1&from=EUR&to=${requestCurrency}`
		);
		res = res.data;
	...
	return (
		<div id="gameDeals">
			...
			{deals && deals.length > 0 ? (
						...
						<table className="headerTable">
							<tbody>
								<tr>
									<th>Store</th>
									<th>Edition</th>
									<th>Price in {currency.symbol}</th>
									<th></th>
								</tr>
							</tbody>
						</table>
				...
				// If there are no deals, render a message indicating that there are no deals
				<div className="no-deals-message">
					<h2>
						Sorry, there are currently no deals available for this
						game.
					</h2>
				</div>
			)}
