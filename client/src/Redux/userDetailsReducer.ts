...
export const getUser = createAsyncThunk('userDetails/updateUser',
	async (user: any, thunkAPI) => {
		try{
			const response = await axios.post("https://gamerank.adarabadian.com/users/login", user);
			localStorage.setItem("token", response.data.token);
			const socket = await io("https://gamerank.adarabadian.com/", {query : {"token" : response.data.token}});
...
export const loginWithToken = createAsyncThunk('userDetails/loginWithToken',
	async (token: any, thunkAPI) => {
		try{
			...
			const response = await axios.post("https://gamerank.adarabadian.com/users/logUserWithToken",token);
			localStorage.setItem("token", response.data.token);
			const socket = await io("https://gamerank.adarabadian.com/", {query : {"token" : response.data.token}});
			response.data.socket = socket;
